...
export function readDisk() {
  ...
  // getDiskInfo() itself wasn't wrapped before (19.08.2026, Jan's
  // real-device test: storage card stayed stuck on the "..." placeholder
  // forever) - unlike readMemory()'s try/catch, an actual throw here
  // (not just "API doesn't exist") would propagate uncaught out of the
  // setInterval callback in page/home/index.js and silently stop that
  // card from ever updating again, with nothing on screen explaining why.
  let d
  try {
    d = getDiskInfo()
  } catch (e) {
